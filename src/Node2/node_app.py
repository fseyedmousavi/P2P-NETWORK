...
def request(next_node : dict, friend_counter : int):
    ...
    reply = requests.get("http://127.0.0.1:" + str(next_node['node_port']) + '/' + file_name + '/' + str(current_node['node_number']))
    if (type(reply) == dict):
        nodes_visited.append(next_node)
        if (nodes_visited.__contains__(reply)):
            request(current_node['friend_nodes'][friend_counter], friend_counter+1)
        else :
            logging.info("another path to node number " + reply['node_name'] + "was introduced")
            request(reply, friend_counter)
    elif (type(reply) == str):
        original = r''+ reply
        target = r'' + 'Node' + current_node['node_number'] + '/newFiles/' + file_name
